            ...
            child: MyButton(
              ...
              onPressed: () async {
                ...
                if(widget.personne.groupeActif != null){
                  await Firestore.instance.collection("groups").document(widget.personne.groupeActif.id).collection('arrets').document(arrets).setData({
                    //'coords': GeoPoint(document['place'].latitude, document['place'].longitude),
                    'coords': GeoPoint(place.latitude, place.longitude),
                    'place': arrets,
                  });
                } else {
                  await Firestore.instance.collection("users").document(widget.personne.compte.email).collection('arrets').document(arrets).setData({
                    'coords': GeoPoint(place.latitude, place.longitude),
                    'place': arrets,
                  });
                }
